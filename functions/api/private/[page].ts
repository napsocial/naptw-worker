...
export async function onRequestGet(context: DefaultRequest) {
    ...
    try {
        const result: PrivateDatabaseResponse | null =
            await context.env.DB.prepare(
                `SELECT original, create_at, expire_at, key_hash, original_hash, create_user FROM private_short_link WHERE short = ? AND enabled = 1 AND (expire_at IS NULL OR expire_at > CURRENT_TIMESTAMP)`,
            )
                .bind(link)
                .first();
        ...
        console.log(e);
        return new Response(ErrorMessages[APIErrorType.ServerError], {
            status: 500,
        });
    }
}
