...
function getTypeShort(link: string): ShortString {
    if (link.startsWith("s:"))
        return { type: ShortType.Secure, short: link.split(":")[1] };
    if (link.startsWith("o:"))
        return { type: ShortType.SanZi, short: link.split(":")[1] };
...
export async function onRequest(context: DefaultRequest) {
    ...
    if (
        ["@react-refresh", "favicon.ico", "robots.txt"].some((val) =>
            link.match(val),
        )
    )
    ...
    try {
        ...
        const SQL =
            shortType.type !== ShortType.SanZi
        ...
        const result: ShortURLDatabaseResponse | null =
            await context.env.DB.prepare(SQL).bind(shortType.short).first();

        const url = new URL(context.request.url);
        const defaultPath = "".concat(url.protocol, "//", url.host);

        if (result === null) return Response.redirect(defaultPath, 302);
        if (shortType.type === ShortType.Secure)
            return Response.redirect(
                defaultPath.concat("/encryption/", shortType.short),
            );

        if (shortType.type === ShortType.Normal && url.searchParams.has("q"))
            return Response.redirect(
                defaultPath.concat("/analysis/", shortType.short),
            );
        if (shortType.type === ShortType.Normal && !url.searchParams.has("dnt"))
            await context.env.DB.prepare(
                "INSERT INTO analysis (`short_link`, `user_agent`, `access_ip`, `country_code`) VALUES (?, ?, ?, ?)",
            )
                .bind(
                    link,
        ...
        console.error(ID, e);
        return new Response(
            ErrorMessages[APIErrorType.ServerError].replace(
                "{request_id}",
                "#" + ID,
            ),
            { status: 500 },
