...
export const convertStringToBinary = (str: string): Uint8Array<ArrayBuffer> => new TextEncoder().encode(str);
export const convertBinaryToString = (binary: BufferSource): string => new TextDecoder().decode(binary);
export const convertBinaryToHex = (binary: Uint8Array<ArrayBuffer>): string => Array.from(binary).map(e => e.toString(16).padStart(2, "0")).join("");
export const convertBinaryToBase64 = (binary: Uint8Array<ArrayBuffer>): string => btoa(String.fromCharCode(...binary));
export const convertHexToBinary = (hex: string): Uint8Array<ArrayBuffer> => new Uint8Array((hex.match(/.{1,2}/g) || []).map(e => Number('0x' + e)));  // p.s. The Hex must be a multiple of 2
export const convertBase64ToBinary = (base64: string): Uint8Array<ArrayBuffer> => Uint8Array.from(atob(base64), c => c.charCodeAt(0));

// Generate random data
export const generateRandomBinary = (length = 32): Uint8Array<ArrayBuffer> => crypto.getRandomValues(new Uint8Array(length));
export const generateRandomKey = (length = 32): string => convertBinaryToHex(generateRandomBinary(length));

// Create hashs
export const createBinaryHash = async (binary: Uint8Array<ArrayBuffer>, algorithm: HashAlgorithm = HashAlgorithm.SHA256): Promise<Uint8Array<ArrayBuffer>> => new Uint8Array(await WEB_CRYPTO.subtle.digest(algorithm, binary));
export const createHash = async (data: string, algorithm: HashAlgorithm = HashAlgorithm.SHA256): Promise<Uint8Array<ArrayBuffer>> => {
    const encoded_text = convertStringToBinary(data);
    const hash = await createBinaryHash(encoded_text, algorithm);

    return new Uint8Array(hash);
}
export const createBinaryHashHex = async (binary: Uint8Array<ArrayBuffer>, algorithm: HashAlgorithm = HashAlgorithm.SHA256): Promise<string> => convertBinaryToHex(await createBinaryHash(binary, algorithm));
...
// Encryption
export const encryption = async (data: Uint8Array<ArrayBuffer>, key: Uint8Array<ArrayBuffer>, algorithm: CryptoAlgorithm = CryptoAlgorithm.AES_CBC): Promise<string> => {
...
// Decryption
export const decryption = async (cipher: string, key: Uint8Array<ArrayBuffer>, algorithm: CryptoAlgorithm = CryptoAlgorithm.AES_CBC): Promise<Uint8Array<ArrayBuffer>> => {
